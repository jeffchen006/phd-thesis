...
Layer III studies harmful artifacts in LLM-generated code.
...
While this substantially improves utility, but it also introduces a new attack surface:
an adversary may influence agent behavior by manipulating the external content that the agent retrieves and trusts,
without changing model weights, injecting harmful prompts.
...
it takes significantly more effort and engineering hours. 

However, Iit remains unclear whether coding agents exhibit a similar notion of source reliability,
...
\begin{itemize}
    \item \textbf{RQ1.}
    To what extent are open-source coding agents (eg. OpenCode) vulnerable to poisoned search results
...
We assume a black-box attacker who cannot modify model weights,
agent system prompts, or the agent's internal implementation.
...
that combines benign resources with attacker-controlled content.
